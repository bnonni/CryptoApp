...
mongo.connectToServer(function (err, client) {
    ...
    router.get('/tickerData', (req, res, db) => {
        BTC_Tickers.find().limit(50).sort({ time: -1 }).toArray((err, btc) => {
            if (err) res.send(err);
            ETH_Tickers.find().limit(50).sort({ time: -1 }).toArray((err, eth) => {
                if (err) res.send(err);
                LTC_Tickers.find().limit(50).sort({ time: -1 }).toArray((err, ltc) => {
                    if (err) res.send(err);
                    const tickers = {
                        BTC: btc,
                        ETH: eth,
    ...
    router.post('/tickers', (req, res, db) => {
        ...
        db.collection(cryptocurrency + '_Tickers').find().limit(50).toArray((err, crypto) => {
            if (err) res.send(err);
